 ...
 * @param {string} originData 原始star数据
 * @param {string} token Github的token
 * @returns 返回月度star数和月度总star数
 */
const getGithubStarCount = async (organizationName, repo, originData = {}, token = '') => {
  let output = {
    repoName: repo,
    monthlyStars: originData.monthlyStars || {},
    monthlyTotalStars: originData.monthlyTotalStars || {},
    starCount: originData.starCount || 0
  };

  let startUpdateDate = dayjs('2010-01');
  // 在已有数据的基础上采集新数据，采集标准是去掉最后一个月的数据
  if (Object.keys(output.monthlyTotalStars).length > 1) {
    const monthStarKeyList = Object.keys(output.monthlyTotalStars);
    const monthStarValueList = Object.values(output.monthlyTotalStars);
    const lastMonth = monthStarKeyList[monthStarValueList.length - 1];
    output.starCount = monthStarValueList[monthStarValueList.length - 2];
    startUpdateDate = dayjs(lastMonth);
    delete output.monthlyTotalStars[lastMonth];
    delete output.monthlyStars[lastMonth];
  } else {
    output.starCount = 0;
    output.monthlyStars = {};
    output.monthlyTotalStars = {};
  }

  let needNextPage = true;
  let pageSize = 100;
  let page = Math.floor(output.starCount / pageSize) + 1;
  let totalStars = output.starCount;
  ...
  while (needNextPage) {
    try {
      ...
      data.forEach(({ starred_at: starredAt }) => {
        ...
        if (startUpdateDate.isAfter(dayjs(date))) {
          return;
        }
 ...
 * @returns
 */
const fetchOrganizationRepoDetail = async (repoList, organizationName, originRepoDetailList, token) => {
  const repoDetailList = [];
  for (const repo of repoList) {
    const repoName = repo.name.split('/')[1];
    let originData = {};
    const originDetail = originRepoDetailList.find((item) => item.repoName === repoName);
    if (originDetail) {
      originData = originDetail;
    }
    const output = await getGithubStarCount(organizationName, repoName, originData, token);
